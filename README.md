...
马里文共和国是一个虚构的南太平洋发展中国家。本仓库把世界观资料、公开数据、确定性模拟模型和多个国家网站放在同一套可追溯体系中：天气、汇率、商品价格、CPI、人口、GDP 与登革热会在每日 Tick 中依次更新，相同状态与种子始终产生相同结果。
...
Mariven Republic is a fictional South Pacific country implemented as a reproducible living-nation model. The repository combines 84 worldbuilding documents, public datasets, government-facing web prototypes, and a deterministic Python simulation engine. Seven P0 models currently run in the daily pipeline: weather, foreign exchange, commodity prices, CPI inflation, a full single-age population cohort model, quarterly GDP national accounts, and a complete age-structured dengue model.
...
P0 已完成 7/8。七个已实现模型均由 [`sim-engine/engine/engine.py`](sim-engine/engine/engine.py) 的每日主循环统一编排。
...
| 登革热 | ✅ | 七省×五年龄组×四血清型、4-bit 免疫史、人群 SEIR/蚊媒 SEI、监测延迟与周版本 | 估计感染、报告/确诊病例、医疗压力、预警与 wMar-1 试点效果 |
| 犯罪 | ⬜ | 规划中 | 分类案件、风险与事件 |
...
登革热模型使用整体 schema v5 中的独立版本化状态，并与权威人口账本逐日对账：

- 人群状态覆盖七省、五个年龄组、DENV-1 至 DENV-4 四种血清型，以及 4-bit 终身感染史掩码。
- 康复后进入 180 天临时交叉保护环形队列，同时保留 10% 剩余易感性；同一人同一天最多感染一种血清型。
- 蚊媒按省维护归一化 SEI 状态，受温度、湿度、14 日降雨、暴雨冲刷、省际接触矩阵和季节化境外输入压力影响。
- 临床树区分感染、症状、警示征象、重症、住院和死亡；报告、抽样、实验室确认与真实感染是不同账本。
- 流行病学周为周一至周日，周报在周末后第 4、14、28 天发布临时、修订和最终版本，既有版本不会被覆盖。
- 省级基线、观察、警报、暴发和恢复状态按周推进；全国医疗压力连续两周超过阈值或多省同时暴发时启动全国紧急状态。
- wMar-1 仅在卡托拉和西部省的小范围试点生效；它是马里文项目的虚构设定，不是真实世界干预证据。
- 固定种子 42 的 2026 中央情景累计报告 1,589 例；12 种子中位数保持在预定的 1,500–2,000 例区间，且配对反事实中试点降低了试点省平均传播。
- 登革热只提交分省、分年龄死亡请求；实际人口扣减由人口模型统一确认，避免与当日基础死亡重复计算。
- v4 旧状态若晚于 2026-08-11 权威锚点，会使用季节气候基线和专用 `migration_replay` 随机流逐日重放；重放恢复内部账本但不补发历史事件。

公开对象位于 `public_health.dengue`。其中 `estimated_infections` 是模型估计感染，`reported_cases` 是经过就医与报告延迟后进入监测系统的病例，两者不可互换。公开监测还提供近 4 周报告数、简化周病例比值 Rt 及区间、当日检测阳性率、病死比、省级预警与全国紧急状态；Rt 是运行监测指标，不是正式流行病学后验估计。

设计说明见 [`docs/superpowers/specs/2026-07-19-complete-dengue-model-design.md`](docs/superpowers/specs/2026-07-19-complete-dengue-model-design.md)。
...
flowchart LR
    ...
    J --> K["刷新人均GDP"]
    K --> L["schema v5 完整验证"]
    L --> M["JSON 快照与 SQLite 索引"]
```

各模型使用隔离的命名随机流。整体状态升级到 schema v5 后，天气、汇率、商品、CPI 和既有事件仍保留 schema v2 随机序列，人口保留 schema v3 随机序列，登革热使用 schema v4 命名流；GDP 本身不消耗随机流，避免新模型导致历史结果漂移。
...
当前测试覆盖确定性、随机流隔离、schema v1/v2/v3/v4→v5 迁移、严格 JSON、断点恢复、死亡去重、年龄推进、365 天人口精确账本、GDP 双重核算与发布版本、登革热中央情景、12 种子配对反事实、10 年浸泡、2035 长期人口演化，以及 dry-run 不落盘。
...
| 登革热疾病与监测框架 | WHO、WHO 西太平洋区域办公室 | 潜伏期、病程、病例定义、区域合理性和报告限制 |
| 南太平洋暴发尺度交叉核对 | Fiji Ministry of Health and Medical Services | 仅作空间异质性与数量级合理性检查，不复制为马里文病例 |
...
1. [`sim-engine/data/sources/dengue_external_anchors_2026.json`](sim-engine/data/sources/dengue_external_anchors_2026.json) 区分 WHO/Fiji 官方外部参考、马里文项目设定与虚构 wMar-1 干预。
2. [`sim-engine/scripts/build_dengue_baseline.py`](sim-engine/scripts/build_dengue_baseline.py) 生成七省人口、免疫先验、传播、临床、监测、医疗容量和干预参数。
3. [`sim-engine/data/dengue_baseline_2026.json`](sim-engine/data/dengue_baseline_2026.json) 是 Tick 离线读取的版本化运行产物，并可由构建脚本字节级重新生成。

```bash
python sim-engine/scripts/build_dengue_baseline.py \
  --output sim-engine/data/dengue_baseline_2026.json
```

现实 Fiji 和 WHO 数据只提供疾病机制、区域数量级与监测方法参考；1,240 例历史锚点、七省分布、2026 中央情景和 wMar-1 均属于马里文模型设定或校准结果，不是现实国家观测。
...
- [登革热公共接口与编排](sim-engine/engine/dengue_model.py)
- [登革热人群与蚊媒动力学](sim-engine/engine/dengue_dynamics.py)
- [登革热临床与监测](sim-engine/engine/dengue_surveillance.py)
